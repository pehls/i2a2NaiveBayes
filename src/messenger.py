...
class Messenger(object):
    ...
    def send_balance_telegram(self, balance_items, previous_total_balance):
        """
        Used to send a user balance telegram message
        ...
        :rtype: float
        """
        telegram_emoji = self.telegram_str["balance"]["emoji"] * 8 + "\n"
        telegram_message = telegram_emoji + self.telegram_str["balance"]["header"]
        total_balance = 0

        for balance in balance_items:
            sub_header = self.telegram_str["balance"]["subHeader"]
            if not balance["IsTracked"] and balance["Currency"] != "BTC":
                sub_header = self.telegram_str["balance"]["subHeaderUntracked"]
            telegram_message += sub_header.format(balance["Currency"])

            if balance["Currency"] != "BTC":
                telegram_message += self.telegram_str["balance"]["balance"].format(balance["Balance"], balance["Currency"])

            telegram_message += self.telegram_str["balance"]["btcValue"].format(balance["BtcValue"])
        ...
        telegram_message += self.telegram_str["balance"]["subHeaderTotal"].format("Total Balance", percentage_change_str) + (
            self.telegram_str["balance"]["btcValue"].format(round(total_balance, 8))
        )

        self.send_telegram(telegram_message)
        return total_balance

    def send_buy_telegram(self, coin_pair, rsi, day_volume):
        """
        Used to send a buy's telegram message
        ...
        telegram_emoji = self.telegram_str["buy"]["emoji"] * 8 + "\n"
        telegram_message = telegram_emoji + self.telegram_str["buy"]["message"].format(coin_pair, ceil(rsi), floor(day_volume),
                                                                              main_market)
        self.send_telegram(telegram_message)

    def send_sell_telegram(self, coin_pair, rsi, profit_margin):
        """
        Used to send a sale's telegram message
        ...
        telegram_emoji = self.telegram_str["sell"][emoji_type] * 8 + "\n"
        telegram_message = telegram_emoji + self.telegram_str["sell"]["message"].format(coin_pair, floor(rsi),
